...
lfCompare <- compareLockFiles(
  filename1 = "E:/git/ohdsi-studies/StrategusStudyRepoTemplate/renv.lock",
  filename2 = "E:/git/itx-asj/ohda_strategus_project_template/renv.lock"
)
head(lfCompare)
diffsInLfCompare <- lfCompare[lfCompare$lockfile1Version != lfCompare$lockfile2Version,]
diffsInLfCompare <- diffsInLfCompare[!is.na(diffsInLfCompare$lockfile1Name),]

# Print out the renv::record syntax
cranPackages <- diffsInLfCompare[diffsInLfCompare$lockfile2Source == "Repository",]
foo <- glue::glue("list(
                      Package = \"{cranPackages$lockfile1Name}\",
                      Version = \"{cranPackages$lockfile2Version}\",
                      Source = \"Repository\",
                      Repository = \"CRAN\"
                    )")
bar <- unlist(paste0(foo, ","))
gitHubPackages <- diffsInLfCompare[diffsInLfCompare$lockfile2Source != "Repository",]
foo <- glue::glue("OHDSI/{gitHubPackages$lockfile1Name}@{gitHubPackages$lockfile2Version}")
bar